...
function startPercentLoop() {
  ...
  const interval = setInterval(() => {
    ...
    const randomMinDamage = DAMAGE_PER_TICK + Math.round(Math.min(Math.random(), Math.random()) * 10);
    const randomMaxDamage = DAMAGE_PER_TICK + Math.round(Math.max(Math.random(), Math.random()) * 20);
